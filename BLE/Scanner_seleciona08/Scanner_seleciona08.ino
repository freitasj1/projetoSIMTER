...
void conexao(int x) {
  ...
  if (pClient->isConnected()) {
    ...
    Serial.println("começando a ler");
    delay(200);
    //for (int i = 0; i < L; i++){

    BLERemoteCharacteristic* pRemoteCharacteristic = pRemoteService->getCharacteristic(BLEUUID(CHARACTERISTIC_UUID[x]));
    Serial.println("leu!!!!");
...
void loop() {
  ...
  if (myDevice) {
    while (i < 2) {
      conexao(i);
      delay(1000);
      Serial.println("Mudando de dispositivo");
      i++;
    }
  } else {
