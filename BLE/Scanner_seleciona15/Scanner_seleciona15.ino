...
BLERemoteCharacteristic* comparar(BLEClient* conectados) {
  for (int i = 0; i < ptrachados.size(); i++) {
    ...
    for (int j = 0; j < L; j++) {
      BLERemoteCharacteristic* pRemoteCharacteristic = ptrachado[i]; 
...
void loop() {
  ...
  if (currentM - startM >= 5000) {
    for (int c = 0; c < conectados.size(); c++) {
      BLERemoteCharacteristic* pRemoteCharacteristic = comparar(conectados[c]);
