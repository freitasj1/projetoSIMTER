...
bool conexao(BLEAdvertisedDevice* myDevice,BLERemoteService *myService) {
  Serial.println("conectando ao dispositivo... ");
  Serial.println(myDevice->toString().c_str());

  BLEClient* pClient = BLEDevice::createClient();
  conectados.push_back(pClient);
  Serial.println("empurrou");
  ...
  if(pClient->connect(myDevice)){
    ...
    Serial.println("Serviçou");
    myService=pRemoteService;
    return true;
...
void setup() {
  ...
  for (int i = 0; i < achados.size(); i++) {
    ...
    conexao(achados[i],ptrachados[i]);
  }
}
